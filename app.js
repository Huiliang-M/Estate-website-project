...
app.post('/listings', (a, b) => {
    connection.query('DELETE FROM Estates WHERE Address =' + connection.escape(a.body.val), c => {
        if (c) throw c; 
        cloudinary.uploader.destroy(a.body.val.replace(/ /g,"_"),function(e,f){console.log(f)}),
...
// dashboard
    ...
    connection.query('SELECT * FROM Estates WHERE Email = ?', [a.session.user.email], function(c, d) {
        ...
        g++)f.push({
            ...
            }), Address:d[g].Address, City:d[g].City, Zip:d[g].Zip, Price:d[g].Price, Bedrooms:d[g].Bedrooms, Bathrooms:d[g].Bathrooms
        });
        b.render('dashboard', {
...
// logout
    ...
    a.session.destroy(() => {}), b.redirect("/fa17g03/login")
// Wire request 'post' actions
